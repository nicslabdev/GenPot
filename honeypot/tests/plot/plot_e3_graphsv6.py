...
# =======================
# Compute global y-ranges per metric
# =======================
y_ranges = {}
for label, col in metrics.items():
    vals = df_sorted[col].dropna()
    if len(vals):
        y_ranges[label] = (np.nanmin(vals) * 0.95, np.nanmax(vals) * 1.05)
    else:
        y_ranges[label] = (0, 1)

# =======================
# Create 3×6 grid (wider)
# =======================
...
for col_idx, model in enumerate(model_order):
    ...
    for row_idx, (label, colname) in enumerate(metrics.items()):
        ...
        # apply same y-scale across models for this metric
        ymin, ymax = y_ranges[label]
        ...
        if row_idx == len(metrics) - 1:
            ax.set_xlabel("Time (min)", fontsize=9.2)
        else:
            ax.set_xlabel("")
        if row_idx == 0:
...
plt.savefig("fig_resources_comparative_3x6_sameY.png", dpi=400, bbox_inches="tight")
plt.show()

print("✅ Saved figure: fig_resources_comparative_3x6_sameY.png")
