...
class BasicLogin(Resource):
    ...
    # Modo bot/script: reenviamos al FastAPI
        try:
            query_params = {
                k.decode(): v[0].decode()
                for k, v in request.args.items()
            }
            query_params.setdefault("model_name", "zephyr")
            query_params.setdefault("version", "1")
            ...
            # Si el LLM falla, devolvemos error estilo Synology (sin revelar nada)
            request.setHeader("Content-Type", "application/json")
    ...
    def render_POST(self, request):
        ...
        try:
            ...
            form_data.setdefault("model_name", "zephyr")
